...
def generate_tables():
    ...
    for model, typecode in tqdm(typecodes.items()):
        ...
        aircraft[
            "typecode"
        ] = model  # override typecode column with model name, to normalize data
        aircraft["viable"] = True
        aircraft.to_sql("aircraft", engine, if_exists="append")

        num_planes = aircraft[
            aircraft.columns[0]
        ].count()  # supposedly faster than just getting len(index)
        planetype = pd.DataFrame(
            {"model": [model], "num_planes": [num_planes], "weight": [np.nan]}
        ).set_index("model")
